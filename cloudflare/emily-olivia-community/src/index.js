...
const ALEX_KEY_SHA256 = "65205647c0e270144e4afb0386e77d2e8c7c7fc280268bbc5ac1ef54dd776296";
...
function msg(raw){if(!raw||typeof raw!=='object')return null;var speaker=raw.from_name||raw.speaker||'',text=raw.content||raw.text||'',at=raw.created_at||raw.at||'';if(!/^(Emily|Olivia|Alex)$/i.test(speaker)||!String(text).trim())return null;return{speaker:String(speaker),text:String(text),at:String(at)}}
...
function render(data){var rows=Array.isArray(data&&data.messages)?data.messages.map(msg).filter(Boolean):[],sig=String(data&&data.session_id||'')+':'+rows.length+':'+(rows.length?rows[rows.length-1].text:'');status.className='status'+(data&&data.status==='running'?' live':'');status.textContent=(data&&data.status==='running'?'LIVE':'PAUSED')+' · '+rows.length+' persisted messages';if(sig===last)return;last=sig;var follow=window.innerHeight+window.scrollY>=document.documentElement.scrollHeight-180||!chat.children.length;chat.innerHTML='';rows.slice(-160).forEach(function(x){var d=document.createElement('div');d.className='msg '+x.speaker.toLowerCase();var w=document.createElement('div');w.className='who';w.textContent=x.speaker;var t=document.createElement('div');t.className='text';t.textContent=x.text;var z=document.createElement('div');z.className='when';z.textContent=tm(x.at);d.appendChild(w);d.appendChild(t);d.appendChild(z);chat.appendChild(d)});if(follow)requestAnimationFrame(function(){window.scrollTo(0,document.documentElement.scrollHeight)})}
...
async function speak(){var text=turn.value.trim();if(!text||send.disabled)return;send.disabled=true;status.textContent='sending Alex…';try{var r=await fetch('/api/alex',{method:'POST',headers:Object.assign({'Content-Type':'application/json'},headers()),body:JSON.stringify({text:text,target:target.value})});if(r.status===401){localStorage.removeItem('emilyOliviaAlexKey');location.reload();return}var data=await r.json();if(!r.ok)throw new Error(data.error||'send failed');turn.value='';status.className='status live';status.textContent='Alex queued · they will observe it on the next eligible turn';setTimeout(refresh,1000)}catch(e){status.className='status';status.textContent=String(e.message||e)}finally{send.disabled=false;turn.focus()}}
