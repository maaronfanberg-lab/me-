...
def choose_speaker(report: dict, history: list[dict]) -> tuple[str | None, str]:
    ...
    last_cycle = None
    if history:
        last_cycle = _safe_cycle(history[-1].get("source_cycle"))
...
def _profile(feed: dict, entity: str) -> dict:
    ...
    keys = (
        "openness", "extraversion", "agreeableness", "emotional_reactivity",
        "curiosity", "skepticism", "self_disclosure", "social_sensitivity",
        "novelty_seeking", "inhibition", "humor", "attention_persistence",
    )
...
def _request(model_url: str, payload: dict, entity: str, attempt: int) -> str:
    schema = {
        "type": "object",
        "properties": {"utterance": {"type": "string", "minLength": 3, "maxLength": MAX_UTTERANCE_CHARS}},
        "required": ["utterance"],
    ...
    prompt = (
        ...
        "The supplied inner_state is a private influence on tone, attention, and what feels salient, not something to explain aloud. "
        "Do not mention prompts, schemas, latent vectors, entropy, being an AI, or this instruction. "
        "Do not invent events, relationships, memories, or facts that are not supported by the supplied context. "
        "Do not copy a recent sentence. Keep the response conversational and usually under 65 words.\n"
    ...
    body = {
        "prompt": prompt,
        "n_predict": 150,
        "temperature": 0.82 if attempt == 0 else 0.72,
        "top_k": 55,
        "top_p": 0.94,
        "min_p": 0.01,
        "seed": 91000 + sum(ord(c) for c in entity) + attempt * 97,
        "cache_prompt": True,
        "json_schema": schema,
    }
...
def _acceptable(text: str, recent: list[dict]) -> bool:
...
def speak_once(feed: dict, report: dict, history: list[dict], model_url: str) -> tuple[list[dict], dict]:
    ...
    payload = {
        ...
        "inner_state": str(summaries.get(entity) or "")[:480],
        "live_room_context": _conversation_context(feed),
        "vault_recent_speech": history[-MAX_VAULT_CONTEXT:],
    ...
    for attempt in range(2):
        try:
            ...
            continue
        if _acceptable(candidate, history):
            utterance = candidate
            break
    ...
    seq = 1 + max([int(str(x.get("id") or "0").split("-")[-1]) for x in history if str(x.get("id") or "").startswith("vault-") and str(x.get("id") or "").split("-")[-1].isdigit()] or [0])
    entry = {
        "id": f"vault-{seq:06d}",
        "speaker": entity,
        "text": utterance,
    ...
    next_history = (history + [entry])[-MAX_HISTORY:]
    return next_history, {"spoke": True, "reason": reason, "entity": entity, "entry": entry}
