...
def _postflight() -> None:
    ...
    history_raw=vault_talker._load(history_path, [])
    clean, stats=history_sanitizer.sanitize_history(history_raw)
    ...
    firewall=room2_runtime_firewall.validate(
        vault_talker._load(feed_path,{}), report, clean,
        vault_talker._load(result_path,{}), stats, heartbeat,
    )
    firewall_path=history_path.parent/'firewall-report.json'
    vault_talker._atomic_json(firewall_path, firewall)
