...
def patch_gpt() -> None:
    ...
    text = path.read_text(encoding="utf-8")
    marker = "COMMUNITY_BITNET_HTTP_V2"
    if marker in text:
        return

    # Replace Stanford's external generation section as one unit.  The prior
    # patch intentionally removed OpenAI request functions but accidentally
    # removed chat_safe_generate too, which memory/reflection/interaction use.
    # Keep that public helper while routing every text generation through the
    # already-loaded localhost BitNet server.
...
def main() -> None:
    ...
    patch_memory()
    patch_interaction()
    print("Stanford runtime patched for persistent local BitNet HTTP generation.")
