...
def patch_gpt() -> None:
    ...
    text = path.read_text(encoding="utf-8")
    marker = "COMMUNITY_BITNET_RAW_V5"
    if marker in text:
        return

    # The Stanford prompt is already a complete interaction prompt. Sending it
    # through an OpenAI-style user/assistant chat wrapper biases Falcon toward
    # assistant/customer-service behavior. Preserve the Stanford prompt exactly
    # and ask llama.cpp to continue it as raw text instead.
...
def main() -> None:
    ...
    patch_memory()
    patch_interaction()
    print("Stanford runtime patched for persistent local BitNet raw generation.")
