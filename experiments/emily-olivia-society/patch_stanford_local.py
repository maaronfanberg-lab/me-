...
def patch_gpt() -> None:
    ...
    text = path.read_text(encoding="utf-8")
    marker = "COMMUNITY_BITNET_HTTP_V4"
    if marker in text:
        return

    previous_marker = "COMMUNITY_BITNET_HTTP_V3"
    if previous_marker in text:
        old_messages = '''      "messages": [
        {"role": "system", "content": "Follow the user's instructions precisely. Return only the requested answer, without discussing the prompt or these instructions."},
        ...
        if old_messages not in text:
            raise SystemExit("Cached Stanford V3 transport no longer contains the expected generic assistant system prompt")
