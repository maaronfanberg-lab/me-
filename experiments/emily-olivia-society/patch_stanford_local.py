...
def patch_gpt() -> None:
    ...
    text = path.read_text(encoding="utf-8")
    marker = "COMMUNITY_BITNET_HTTP_V3"
    if marker in text:
        # Earlier Community workflows capped generation at 64 tokens. Stanford's
        # interaction prompt asks for a JSON object, and run #73 proved that the
        # cap can truncate the object before its closing quote/brace. Keep the
        # workflow value as a requested budget but guarantee enough room for one
        # complete Stanford response. This is transport only, not dialogue logic.
        old_cap = '"max_tokens": min(int(max_tokens), int(os.environ.get("COMMUNITY_MAX_TOKENS", "96"))),'
        new_cap = '"max_tokens": min(int(max_tokens), max(128, int(os.environ.get("COMMUNITY_MAX_TOKENS", "96")))),'
        if old_cap in text:
            path.write_text(text.replace(old_cap, new_cap, 1), encoding="utf-8")
        return
...
def main() -> None:
    ...
    patch_memory()
    patch_interaction()
    print("Stanford runtime patched for persistent local BitNet chat generation.")
