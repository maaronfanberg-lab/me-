...
def maybe_reflect(agent, time_step: int) -> bool:
    ...
    Blank, malformed, prompt-shaped, and question-shaped reflection output is
    removed rather than stored. When a stochastic pass produces no clean
    insight, resample the same evidence and anchor at the same logical timestep.
    The reflection watermark advances only after a clean reflection exists, so
    a bad local-model draw cannot silently suppress future reflection.
    """
