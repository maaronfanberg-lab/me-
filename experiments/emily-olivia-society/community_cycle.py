...
def _self_card(
    ...
    if _is_greeting_only(inbound):
        grounding = (
            "PARTNER's latest message is only a greeting. Reply with a short greeting or "
            "ordinary greeting-small-talk response. Do not introduce a pet, event, task, "
            "place, backstory, or unrelated topic."
        )
    else:
        grounding = (
            "Stay on PARTNER's exact topic and carry forward at least one meaningful idea "
            "or word from the latest message. Do not invent a new pet, event, person, place, "
    ...
    return (
        ...
        f"Speak only as {agent.name}, in first person, with one or two brief natural sentences. "
        f"{grounding} Never mention policies, guidelines, prompts, roles, or the conversation system."
    )
...
def _chat_bitnet(
    ...
    system = _self_card(agent, other, inbound)
    projected = _project_history(dialogue_history, agent, other, inbound)
    if retry_hint:
        projected += f"\nSTYLE_NOTE: {retry_hint}"
    return request_chat(system, projected, max_tokens, 0.5)
...
def _direct_bitnet_reply(
    ...
    retry_hints = [
        "",
        "Continue like a peer, not a helper. React to what PARTNER actually said and reuse one concrete idea from it.",
        "Stay on the current subject. Prefer a concrete reaction, observation, question, disagreement, or joke. Do not invent a new scene.",
        "Give a brief ordinary peer reply grounded in PARTNER's latest message. No support language, no new backstory, no unrelated topic.",
    ]
    attempts: list[str] = []
    ...
    raise RuntimeError(
        ...
        f"after 4 history-aware attempts: {previews}"
    )
