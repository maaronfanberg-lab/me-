...
# Unsupported specificity is deliberately limited to concrete biography. A
# first-person sentence is not automatically a biography: ordinary thoughts,
# uncertainty and present-moment activity must remain speakable.
...
def _is_unsupported_concrete_biography(
    text: str,
    dialogue_history,
    inbound: str,
    cognitive_context: str,
) -> bool:
    """Reject invented concrete biography, not generic first-person speech."""
    candidate = set(content_tokens(text))
    if len(candidate) < 2 or not _CONCRETE_AUTOBIOGRAPHY.search(str(text or "")):
        return False
    support_text = " ".join(
        [str(inbound or ""), str(cognitive_context or "")]
        + [str(prior) for _speaker, prior in list(dialogue_history or [])[-8:]]
    )
...
def detect_recurring_content_attractor(messages: list[str]) -> dict | None:
    ...
    if recurring_pairs and (recurring_pairs[0][1] >= 5 or len(recurring_pairs) >= 2):
        return {
            ...
            "recurring_pairs": [{"phrase": " ".join(pair), "count": count} for pair, count in recurring_pairs[:5]],
        }
    if recurring_tokens:
        return {
