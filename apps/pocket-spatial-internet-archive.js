...
var NETLABEL_COLLECTION='netlabels';
var DEFAULT_QUERY='';
...
function buildSearchURL(query){
  var q='collection:('+NETLABEL_COLLECTION+') AND mediatype:(audio) AND licenseurl:http*by*';
  var term=cleanQuery(query);
  if(term)q+=' AND ('+term+')';
  return SEARCH_API+'?q='+encodeURIComponent(q)+
    '&fl%5B%5D=identifier&fl%5B%5D=title&fl%5B%5D=creator&fl%5B%5D=licenseurl&fl%5B%5D=collection'+
...
function isNetlabelsItem(value){
  if(value&&typeof value!=='string'&&typeof value.length==='number'){
    for(var i=0;i<value.length;i+=1){if(String(value[i]||'').toLowerCase()===NETLABEL_COLLECTION)return true;}
    return false;
  }
  return String(value||'').toLowerCase()===NETLABEL_COLLECTION;
}
...
function itemFromMetadata(data){
  ...
  if(!isNetlabelsItem(metadata.collection))return null;
...
function loadIntoPlayer(track,spatialSupported){
  ...
  if(trackLabel)trackLabel.textContent='Internet Archive Netlabels · '+track.title+' · '+track.creator;
  if(playerStatus){if(spatialSupported){playerStatus.textContent='Netlabels MP3 passed the live Web Audio PCM probe. Tap Play, then turn Spatial on.';playerStatus.className='status good';}else{playerStatus.textContent='Netlabels MP3 is loaded for dry playback, but this device did not prove PCM access for the spatial engine.';playerStatus.className='status warn';}}
}

function testTrack(ui,track,control,diagnostic){
  control.disabled=true;control.textContent='TESTING LIVE PCM…';diagnostic.textContent='Testing direct Netlabels MP3 playback and Web Audio PCM side by side for about two seconds…';diagnostic.className='status';
  probeStream(track.audio,function(result){
    ...
    if(result.supported){control.textContent='SPATIAL PCM VERIFIED ✓';diagnostic.textContent='Verified on this device: the Netlabels MP3 advances and Web Audio receives non-flat PCM (max deviation '+result.max_analyser_deviation+').';diagnostic.className='status good';}
...
function renderTracks(ui){
  ...
  if(!state.tracks.length){ui.tracks.appendChild(el('div','status','No Netlabels items with exact CC BY + MP3 were found in this search.'));return;}
  var heading=el('div','status','NETLABELS TEST CATALOG · '+state.tracks.length+' ITEMS');heading.style.marginTop='12px';heading.style.fontWeight='700';ui.tracks.appendChild(heading);
  for(var i=0;i<state.tracks.length;i+=1){
    (function(track){
      ...
      var rights=el('div','status good','Spatial test candidate: Netlabels membership + live exact CC BY metadata + MP3.');rights.style.marginTop='4px';row.appendChild(rights);
      var sourceLink=el('a','','Open original on Internet Archive');sourceLink.href=track.source;sourceLink.target='_blank';sourceLink.rel='noopener';sourceLink.style.display='inline-block';sourceLink.style.marginTop='6px';sourceLink.style.color='inherit';row.appendChild(sourceLink);
      var licenseLink=el('a','','View CC BY license');licenseLink.href=track.licenseurl;licenseLink.target='_blank';licenseLink.rel='noopener';licenseLink.style.display='inline-block';licenseLink.style.marginTop='6px';licenseLink.style.marginLeft='10px';licenseLink.style.color='inherit';row.appendChild(licenseLink);
...
function loadCatalog(ui){
  ui.load.disabled=true;ui.load.textContent='SEARCHING NETLABELS…';setStatus(ui,'Searching Internet Archive Netlabels, then rechecking each item for collection membership, exact CC BY, and MP3…','');
  requestJSON(buildSearchURL(ui.query.value),function(error,data){
    if(error){ui.load.disabled=false;ui.load.textContent='SEARCH OPEN MUSIC';state.tracks=[];renderTracks(ui);setStatus(ui,'Internet Archive search failed: '+error.message,'warn');return;}
    var docs=(data&&data.response&&data.response.docs)||[];
    hydrateDocs(docs,function(items){ui.load.disabled=false;ui.load.textContent='SEARCH OPEN MUSIC';state.tracks=items;renderTracks(ui);if(items.length)setStatus(ui,'Verified Netlabels candidates loaded. Pick one and run the live PCM test on this device.','good');else setStatus(ui,'Netlabels search answered, but none of the returned items survived all license + MP3 checks.','warn');});
  });
}

function createUI(){
  var hero=document.querySelector('.card.hero');if(!hero||!hero.parentNode)return null;
  var card=el('div','card');card.id='archiveCard';var title=el('div','','INTERNET ARCHIVE NETLABELS · LIVE SPATIAL TEST');title.style.fontWeight='700';title.style.letterSpacing='.06em';title.style.fontSize='12px';card.appendChild(title);
  var status=el('div','status','No login or API key required. Pocket Spatial is restricted to Archive Netlabels items whose live metadata also resolves to exact CC BY and contains an MP3.');status.id='archiveStatus';card.appendChild(status);
  var query=document.createElement('input');query.type='search';query.id='archiveQuery';query.value=DEFAULT_QUERY;query.placeholder='Optional search, e.g. ambient';query.setAttribute('aria-label','Internet Archive Netlabels search');query.style.width='100%';query.style.minHeight='46px';query.style.borderRadius='12px';query.style.border='1px solid #293347';query.style.background='#0b1018';query.style.color='#edf2fb';query.style.padding='10px 12px';query.style.fontSize='16px';query.style.marginTop='10px';card.appendChild(query);
  var load=button('SEARCH OPEN MUSIC',function(){loadCatalog(ui);});load.id='archiveLoad';card.appendChild(load);var tracks=el('div','');tracks.id='archiveTracks';card.appendChild(tracks);
  var note=el('div','legal','');note.appendChild(document.createTextNode('Development test only. The source is restricted to Internet Archive’s Netlabels collection, which hosts freely distributable/streamable netlabel catalogs. Pocket Spatial still re-fetches each item’s metadata and only enables exact CC BY candidates. Audio streams directly from archive.org and is never copied or proxied.'));card.appendChild(note);
  hero.parentNode.insertBefore(card,hero);return {card:card,status:status,query:query,load:load,tracks:tracks};
}

function boot(){var ui=createUI();if(!ui)return;root.PocketSpatialInternetArchive={buildSearchURL:buildSearchURL,isExactCCBY:isExactCCBY,isNetlabelsItem:isNetlabelsItem,selectMP3:selectMP3,itemFromMetadata:itemFromMetadata,probeStream:probeStream,getLastProbe:function(){return state.lastProbe;}};}
