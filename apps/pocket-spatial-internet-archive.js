...
function cleanQuery(value){
  var q=String(value||DEFAULT_QUERY).replace(/[^a-zA-Z0-9 _-]+/g,' ').replace(/\s+/g,' ').replace(/^\s+|\s+$/g,'');
  return q||DEFAULT_QUERY;
}

function buildSearchURL(query){
  var q='mediatype:(audio) AND licenseurl:(http*creativecommons.org/licenses/by/*) AND ('+cleanQuery(query)+')';
...
function requestJSON(url,callback){
  ...
  xhr.onreadystatechange=function(){
    ...
    var data=null;
    try{data=JSON.parse(xhr.responseText);}catch(e){finish(new Error('archive_invalid_json'));return;}
    finish(null,data);
  };
...
function firstValue(value){
  if(value&&typeof value!=='string'&&typeof value.length==='number')return value.length?String(value[0]):'';
  return String(value||'');
}
...
function itemFromMetadata(data){
  ...
  var identifier=String(metadata.identifier||'');
  var file=selectMP3(data.files);
  if(!identifier||!file||!file.name)return null;
  return{
    identifier:identifier,
    title:firstValue(metadata.title)||identifier,
    creator:firstValue(metadata.creator)||'Unknown creator',
    licenseurl:firstValue(metadata.licenseurl),
    audio:DOWNLOAD_BASE+encodeURIComponent(identifier)+'/'+encodePath(file.name),
    source:'https://archive.org/details/'+encodeURIComponent(identifier),
    filename:String(file.name),
    format:String(file.format||'MP3')
  };
}
...
function hydrateDocs(docs,callback){
  ...
  for(var i=0;i<docs.length;i+=1){
    (function(doc){
      ...
      if(!id){done();return;}
      requestJSON(METADATA_API+encodeURIComponent(id),function(error,data){
        if(!error){var item=itemFromMetadata(data);if(item)results.push(item);}
        done();
      });
    }(docs[i]));
  }
...
function probeStream(url,callback){
  ...
  function cleanup(){
    ...
    removeNode(nativeMedia);removeNode(tappedMedia);
  ...
  try{
    ...
    analyser.fftSize=256;analyser.smoothingTimeConstant=0;
  ...
  safePlay(nativeMedia);safePlay(tappedMedia);
  timer=setInterval(function(){
    ticks+=1;
    try{
      analyser.getByteTimeDomainData(data);
      for(var i=0;i<data.length;i+=1){var d=Math.abs(data[i]-128);if(d>maxDeviation)maxDeviation=d;}
    }catch(e){}
    if(ticks>=20){
...
function loadIntoPlayer(track,spatialSupported){
  ...
  if(spatialSupported)player.crossOrigin='anonymous';else player.removeAttribute('crossorigin');
  ...
  if(playerStatus){
    if(spatialSupported){playerStatus.textContent='Internet Archive MP3 passed the live Web Audio PCM probe. Tap Play, then turn Spatial on.';playerStatus.className='status good';}
    else{playerStatus.textContent='Internet Archive MP3 is loaded for dry playback, but this device did not prove PCM access for the spatial engine.';playerStatus.className='status warn';}
  }
}

function testTrack(ui,track,control,diagnostic){
  control.disabled=true;control.textContent='TESTING LIVE PCM…';
  diagnostic.textContent='Testing direct Archive MP3 playback and Web Audio PCM side by side for about two seconds…';diagnostic.className='status';
  probeStream(track.audio,function(result){
    state.lastProbe={identifier:track.identifier,result:result};
    control.disabled=false;loadIntoPlayer(track,result.supported);
...
function renderTracks(ui){
  ...
  for(var i=0;i<state.tracks.length;i+=1){
    (function(track){
      ...
      var name=el('b','',track.title);name.style.display='block';row.appendChild(name);
      var meta=el('span','',track.creator+' · CC BY · '+track.format);meta.style.display='block';meta.style.marginTop='4px';row.appendChild(meta);
      var rights=el('div','status good','Spatial test candidate: item metadata resolves to an exact Creative Commons Attribution license.');rights.style.marginTop='4px';row.appendChild(rights);
...
function loadCatalog(ui){
  ui.load.disabled=true;ui.load.textContent='SEARCHING ARCHIVE…';
  setStatus(ui,'Searching public Internet Archive metadata, then checking each candidate for exact CC BY + MP3…','');
  requestJSON(buildSearchURL(ui.query.value),function(error,data){
    ...
    hydrateDocs(docs,function(items){
      ui.load.disabled=false;ui.load.textContent='SEARCH OPEN AUDIO';state.tracks=items;renderTracks(ui);
      if(items.length)setStatus(ui,'Rights-filtered Archive items loaded. Pick one and run the live PCM test on this device.','good');
      else setStatus(ui,'Archive search answered, but none of the returned items survived exact-license + MP3 verification.','warn');
    });
  });
}
...
function createUI(){
  ...
  var card=el('div','card');card.id='archiveCard';
  var title=el('div','','INTERNET ARCHIVE · NO-ACCOUNT LIVE TEST');title.style.fontWeight='700';title.style.letterSpacing='.06em';title.style.fontSize='12px';card.appendChild(title);
  var status=el('div','status','No login or API key required. Pocket Spatial only offers items whose live Archive metadata resolves to exact CC BY and contains an MP3.');status.id='archiveStatus';card.appendChild(status);
  var query=document.createElement('input');query.type='search';query.id='archiveQuery';query.value=DEFAULT_QUERY;query.setAttribute('aria-label','Internet Archive audio search');query.style.width='100%';query.style.minHeight='46px';query.style.borderRadius='12px';query.style.border='1px solid #293347';query.style.background='#0b1018';query.style.color='#edf2fb';query.style.padding='10px 12px';query.style.fontSize='16px';query.style.marginTop='10px';card.appendChild(query);
  var load=button('SEARCH OPEN AUDIO',function(){loadCatalog(ui);});load.id='archiveLoad';card.appendChild(load);
  var tracks=el('div','');tracks.id='archiveTracks';card.appendChild(tracks);
  var note=el('div','legal','');note.appendChild(document.createTextNode('Development test only. Audio is streamed directly from archive.org, never copied or proxied by Pocket Spatial. The app rechecks the item license metadata before showing a stream candidate and links every item back to its original Archive page.'));card.appendChild(note);
...
function boot(){
  ...
  root.PocketSpatialInternetArchive={buildSearchURL:buildSearchURL,isExactCCBY:isExactCCBY,selectMP3:selectMP3,itemFromMetadata:itemFromMetadata,probeStream:probeStream,getLastProbe:function(){return state.lastProbe;}};
}
