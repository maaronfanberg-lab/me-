...
var DEFAULT_QUERY='electronic music';
...
function requestJSON(url,callback){
  ...
  xhr.onreadystatechange=function(){
    ...
    if(xhr.status<200||xhr.status>=300){
      callback(new Error('openverse_http_'+xhr.status));
      return;
    }
    var data=null;
    try{data=JSON.parse(xhr.responseText);}catch(e){callback(new Error('openverse_invalid_json'));return;}
    callback(null,data);
  };
  xhr.ontimeout=function(){callback(new Error('openverse_timeout'));};
  xhr.onerror=function(){callback(new Error('openverse_network_error'));};
  xhr.send(null);
}

function licenseAllowsSpatial(track){
  var license=String((track&&track.license)||'').toLowerCase();
  return license==='by'||license==='by-sa'||license==='by-nc'||license==='by-nc-sa'||license==='cc0'||license==='pdm';
}

function licenseLabel(track){
  var license=String((track&&track.license)||'').toUpperCase();
  var version=String((track&&track.license_version)||'');
  if(!license)return 'License unavailable';
  if(license==='CC0'||license==='PDM')return license;
...
function testTrack(ui,track,control,diagnostic){
  ...
  if(!licenseAllowsSpatial(track)){
    diagnostic.textContent='Blocked from the spatial test: this license is not in the derivative-permitting allowlist.';
...
function renderTracks(ui){
  ...
  if(!state.tracks.length){
    ui.tracks.appendChild(el('div','status','No compatible CC BY MP3 test tracks were returned.'));
    return;
  }
  ...
  for(var i=0;i<state.tracks.length;i+=1){
    (function(track){
      ...
      var meta=el('span','',(track.creator||'Unknown creator')+' · '+licenseLabel(track));
      meta.style.display='block';
      meta.style.marginTop='4px';
      row.appendChild(meta);

      var rights=el('div','status good','Spatial test eligible by API metadata: CC BY, MP3. Verify the source page before reuse outside this test.');
      rights.style.marginTop='4px';
      row.appendChild(rights);

      var original=sourcePage(track);
      if(original){
        var link=el('a','','Open original source');
        link.href=original;
        link.target='_blank';
        link.rel='noopener';
...
function loadCatalog(ui){
  ...
  ui.load.textContent='SEARCHING OPENVERSE…';
  setStatus(ui,'Searching anonymous Openverse audio for CC BY MP3 music…','');

  requestJSON(buildSearchURL(query),function(error,data){
    ui.load.disabled=false;
    ui.load.textContent='SEARCH OPEN MUSIC';
    if(error){
      state.tracks=[];
      renderTracks(ui);
      setStatus(ui,'Openverse API test failed: '+error.message,'warn');
      return;
    }
    var results=(data&&data.results)||[];
    var eligible=[];
    for(var i=0;i<results.length;i+=1){
      if(licenseAllowsSpatial(results[i])&&mp3URL(results[i]))eligible.push(results[i]);
    }
    state.tracks=eligible;
    renderTracks(ui);
    if(eligible.length){
      setStatus(ui,'Openverse results loaded. Pick a track and run the live PCM test on this device.','good');
    }else{
      setStatus(ui,'Openverse answered, but no compatible CC BY MP3 results survived the local safety filter.','warn');
    }
  });
}
...
function createUI(){
  ...
  card.id='openverseCard';

  var title=el('div','','OPENVERSE · NO-ACCOUNT LIVE TEST');
  title.style.fontWeight='700';
  title.style.letterSpacing='.06em';
  title.style.fontSize='12px';
  card.appendChild(title);

  var status=el('div','status','No login or API key required. This test searches openly licensed MP3 audio through the public Openverse API.');
  status.id='openverseStatus';
  card.appendChild(status);

  var query=document.createElement('input');
  query.type='search';
  query.id='openverseQuery';
  query.value=DEFAULT_QUERY;
  query.setAttribute('aria-label','Openverse music search');
  query.style.width='100%';
  query.style.minHeight='46px';
  query.style.borderRadius='12px';
  query.style.border='1px solid #293347';
  query.style.background='#0b1018';
  query.style.color='#edf2fb';
  query.style.padding='10px 12px';
  query.style.fontSize='16px';
  query.style.marginTop='10px';
  card.appendChild(query);

  var load=button('SEARCH OPEN MUSIC',function(){loadCatalog(ui);});
  load.id='openverseLoad';
  card.appendChild(load);

  var tracks=el('div','');
  tracks.id='openverseTracks';
  card.appendChild(tracks);

  var note=el('div','legal','');
  note.appendChild(document.createTextNode('Development test: results are restricted to CC BY MP3 entries from Freesound through Openverse. Openverse warns that license metadata should be verified at the original source, so every result links back to its source page. No audio is downloaded, copied, or proxied by Pocket Spatial.'));
