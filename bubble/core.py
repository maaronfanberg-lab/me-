...
class SegmentResult:
    ...
    def as_dict(self) -> dict:
        return {
            ...
            "max_wall_mach": self.max_wall_mach,
            "emitted_pressure_coeff": self.emitted_pressure_coeff,
            "solver": self.solver,
            "nfev": self.nfev,
...
def far_field_pressure(t: float, p: BubbleParams, coupling: CouplingRamp) -> float:
    return (
        p.p0
        + p.drive_amplitude * math.sin(2.0 * math.pi * p.drive_frequency * t)
        + coupling.pressure(t)
    )
...
def far_field_pressure_rate(t: float, p: BubbleParams, coupling: CouplingRamp) -> float:
    return (
        ...
        * math.pi
        * p.drive_frequency
        * math.cos(2.0 * math.pi * p.drive_frequency * t)
        + coupling.slope
    )
...
def acceleration(
    ...
    p: BubbleParams,
    model: str,
    coupling: CouplingRamp,
) -> float:
    """Return Rddot for Rayleigh-Plesset or Keller-Miksis."""
    if R <= 0:
        raise ValueError("bubble radius must stay positive")
    ...
    # dQ/dt excluding the acceleration-containing piece of d(-4 mu U/R)/dt.
    dq_no_acc = (
        dpg
        + 2.0 * p.sigma * U / (R * R)
        + 4.0 * p.mu * U * U / (R * R)
        - far_field_pressure_rate(t, p, coupling)
    )
    ...
    rhs = (
        (1.0 + U / p.c) * q / p.rho
        + R * dq_no_acc / (p.rho * p.c)
        - 1.5 * (1.0 - U / (3.0 * p.c)) * U * U
    )
    return rhs / denominator


def emitted_pressure_coefficient(R: float, U: float, A: float, rho: float) -> float:
    """Return K where p_acoustic(r) = K/r in the leading monopole far field."""
    return rho * (2.0 * R * U * U + R * R * A)


def linear_natural_frequency(p: BubbleParams) -> float:
    """Small-amplitude angular resonance frequency including surface tension."""
    numerator = (
        3.0 * p.kappa * (p.p0 - p.pv)
        + (6.0 * p.kappa - 2.0) * p.sigma / p.R0
    )
    if numerator <= 0:
        raise ValueError("equilibrium has no positive linear radial frequency")
    return math.sqrt(numerator / (p.rho * p.R0 * p.R0))


def rayleigh_collapse_time(Rmax: float, rho: float, delta_p: float) -> float:
    """Ideal empty-cavity Rayleigh collapse time."""
    if Rmax <= 0 or rho <= 0 or delta_p <= 0:
        raise ValueError("Rmax, rho and delta_p must be positive")
    return 0.914681 * Rmax * math.sqrt(rho / delta_p)
...
def integrate_segment(
    ...
    *,
    ...
    if primary_solver != "Radau":
        methods.append("Radau")
    ...
    return SegmentResult(
        ...
        U=U_final,
        A=A_final,
        min_R=float(np.min(Rvals)),
        max_R=float(np.max(Rvals)),
        max_abs_U=float(np.max(np.abs(Uvals))),
        max_wall_mach=float(np.max(np.abs(Uvals)) / p.c),
